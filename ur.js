                ...
                try {        
                    switch (subPacketType) {                         
                        case 0: // Robot Mode Data
                                ...
                                offset = offset + 8 ;
                                state.robotModeData.physicalRobotConnected = data.readBooleanBE(offset);
                                offset = offset + 1;
                                ...
                                offset = offset + 1;
                                state.robotModeData.protectiveStopped = data.readBooleanBE(offset);
                                offset = offset + 1;
                                ...
                                state.robotModeData.robotModeDescription = this.robotModes.getText(data.readByteBE(offset));
                                offset = offset + 1;
                                state.robotModeData.controlMode = data.readByteBE(offset);
                                state.robotModeData.controlModeDescription = this.controlMode.getText(data.readByteBE(offset));
                                offset = offset + 1;
                                state.robotModeData.targetSpeedFraction = data.readDoubleBE(offset);
                                offset = offset + 8 ;
                                state.robotModeData.speedScaling = data.readDoubleBE(offset);
                                offset = offset + 8 ;
                                state.robotModeData.targetSpeedFractionLimit = data.readDoubleBE(offset);
                                offset = offset + 8;
                                offset = offset +1;
                        ...
                        case 1: // Joint Data;
                                for (let i = 0; i < state.jointData.length; i++) {
                                    ...
                                    state.jointData[i].positionActualDegree = (state.jointData[i].positionActual * 180 / Math.PI);
                                    offset = offset + 8 ;
                                    state.jointData[i].positionTarget = data.readDoubleBE(offset);
                                    state.jointData[i].positionTargetDegree = (state.jointData[i].positionTarget * 180 / Math.PI);
                                    offset = offset + 8 ;
                                    ...
                                    state.jointData[i].modeDescription = this.jointModes.getText(data.readByteBE(offset));
                                    offset = offset + 1 ;                                                                      
                                }
                                offset = subPacketStart + subPacketLength;
                                break;
                        case 2: //Tool Data
                                ...
                                state.toolData.modeDescription = this.toolMode.getText(data.readByteBE(offset));
                                offset = offset + 1;
                                offset = subPacketStart + subPacketLength;
                                break;
                        case 3: //Masterboard Data
                                ...
                                offset = offset + 4;
                                state.masterboardData.digitalOutputBits = data.readInt32BE(offset);
                                offset = offset + 4;
                                ...
                                offset = offset + 8;
                                state.masterboardData.analogOutputDomain0 = data.readByteBE(offset);
                                offset = offset + 1;
                                state.masterboardData.analogOutputDomain1 = data.readByteBE(offset);
                                offset = offset + 1;
                                ...
                                state.masterboardData.safetymodeDescription = this.safetyMode.getText(data.readByteBE(offset));
                                offset = offset + 1;
                                state.masterboardData.inReduceMode = data.readByteBE(offset);
                                offset = offset + 1;
                                state.masterboardData.euromap67installed = data.readByteBE(offset);
                                offset = offset + 1;
                                if(state.masterboardData.euromap67installed > 0){
                                state.masterboardData.euromapInputBits = data.readInt32BE(offset);
                                offset = offset + 4;
                                state.masterboardData.euromapOutputBits = data.readInt32BE(offset);
                                offset = offset + 4;
                                ...
                                offset = offset + 4;
                                state.masterboardData.operationalModeSelectorInput = data.readByteBE(offset);
                                offset = offset + 1;
                                state.masterboardData.threePositionEnablingDeviceInput = data.readByteBE(offset);
                                offset = offset + 1; 
                                offset = subPacketStart + subPacketLength;                             
                                break;
                        ...
                        case 6: //Configuration Data
                                for (let i = 0; i < 6; i++) {
                                    state.configData.jointLimits[i].jointMinLimit = data.readDoubleBE(offset);
                                    offset = offset +8; 
                                    state.configData.jointLimits[i].jointMaxLimit = data.readDoubleBE(offset);
                                    offset = offset +8;                               
                                }
                                for (let i = 0; i < 6; i++) {
                                    state.configData.jointMaxValues[i].jointMaxSpeed = data.readDoubleBE(offset);
                                    offset = offset +8; 
                                    state.configData.jointMaxValues[i].jointMaxAcceleration = data.readDoubleBE(offset);
                                    offset = offset +8;                               
                                }
                                state.configData.defaultSpeedLimit= data.readDoubleBE(offset);
                                offset = offset +8;
                                state.configData.defaultAccelerationLimit = data.readDoubleBE(offset);
                                offset = offset +8;
                                state.configData.defaultToolSpeedLimit = data.readDoubleBE(offset);
                                offset = offset +8;
                                state.configData.defaultToolAccelerationLimit = data.readDoubleBE(offset);
                                offset = offset +8;
                                state.configData.characteristicSizeTool = data.readDoubleBE(offset);
                                offset = offset +8;
                                for (let i = 0; i < 6; i++) {
                        ...
                        case 8: //Additional Info
                                state.additionalInfo.freedriveButtonPressed = data.readBooleanBE(offset);
                                offset = offset +1;
                                state.additionalInfo.freedriveButtonEnabled = data.readBooleanBE(offset);
                                offset = offset +1;
                                state.additionalInfo.ioEnabledFreedrive = data.readBooleanBE(offset);
                                offset = offset +1;
                                offset = offset +1;
                                offset = subPacketStart + subPacketLength;
                                break;
                        ...
                        case 11: // Tool communication Data should be skipped
                                ...
                                offset = offset +4;
                                state.toolCommunicationInfo.rxIdleChars = data.readInt32BE(offset);
                                offset = offset +4;
                                state.toolCommunicationInfo.txIdleChars = data.readInt32BE(offset);
                                offset = offset +4;
                                offset = subPacketStart + subPacketLength;
                                break;
                        default:
